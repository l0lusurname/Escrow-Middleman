...
export async function handleMarkScammed(interaction) {
  ...
  try {
    ...
    if (!isParticipant) {
      return interaction.reply({
        content: "Only trade participants can report a scam.",
        ephemeral: true,
      });
    }
    ...
    console.error("Mark scammed error:", error);
    if (interaction.deferred) {
      await interaction.editReply({
        content: "An error occurred while reporting the scam.",
      });
    } else {
